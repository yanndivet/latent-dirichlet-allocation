...
def BMM(A, B, K, alpha, gamma):
    ...
    mixproportions = np.zeros((K,num_iters_gibbs))
    for iter in range(num_iters_gibbs):
        ...
        mixproportions[:,iter] = (sk_docs[:,0]+alpha)/(np.sum(sk_docs[:,0])+len(sk_docs[:,0])*alpha) 
    ...
    return perplexity, swk, mixproportions
...
#     perplexity, swk, mixproportions = BMM(A, B, K, alpha, gamma)
#     print(perplexity)
#     I = 20
...
#     # Plot mixing against iterations
#     iters = np.linspace(0, len(mixproportions[0])-1, len(mixproportions[0])).T
#     print(np.sum(mixproportions[:,-1]))
#     for topic in mixproportions:
#         plt.plot(iters,topic)
#     plt.title("Evaluation of Topics with Gibbs Iterations")
